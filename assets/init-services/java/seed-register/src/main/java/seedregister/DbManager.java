...
public class DbManager {
    ...
    private static final String COLLECTION_NAME = "register_collection";
            ...
            synchronized (DbManager.class) {
                if (mongoClientInstance == null) {
                    try {
                        ...
                        System.out.println("Seed-Register: Connecting to MongoDB with: " + host + ":" + port);
                        ...
                        System.out.println("Seed-Register: Successfully connected to MongoDB database '" + DB_NAME + "'.");
                        databaseInstance.runCommand(new Document("ping", 1));
                        System.out.println("Seed-Register: Ping successful.");

                    } catch (Exception e) {
                        System.err.println("Seed-Register: FATAL - Failed to initialize MongoDB connection: " + e.getMessage());
                        e.printStackTrace();
                        throw new RuntimeException("MongoDB initialization failed for seed-register", e);
                    }
                }
            }
        }
    ...
    public static MongoCollection<Document> getRegisterCollection() {
        initializeConnection();
        if (databaseInstance == null) {
            throw new IllegalStateException("Database connection is not initialized for seed-register.");
        }
    ...
    public static void closeConnection() {
        if (mongoClientInstance != null) {
            System.out.println("Seed-Register: Closing MongoDB connection...");
            mongoClientInstance.close();
            mongoClientInstance = null;
            databaseInstance = null;
            System.out.println("Seed-Register: MongoDB connection closed.");
        }
    }
}

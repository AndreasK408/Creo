...
public class DbManager {
    ...
    static {
        try {
            ...
            System.out.println("Successfully connected to MongoDB for register handler.");

            MongoCollection<Document> collection = database.getCollection(COLLECTION_NAME);
            collection.createIndex(new Document("username", 1));
            collection.createIndex(new Document("email", 1));
            System.out.println("Indexes for username and email ensured on " + COLLECTION_NAME + " in " + DB_NAME);
        } catch (Exception e) {
            System.err.println("FATAL: Failed to initialize MongoDB connection for register: " + e.getMessage());
            throw new RuntimeException("MongoDB initialization failed for register", e);
        }
    }

    public static MongoCollection<Document> getRegisterCollection() {
        if (database == null) {
            throw new IllegalStateException("Database connection is not initialized for register.");
        }
